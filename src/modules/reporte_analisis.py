...
def AnalisDePortadoras(TipoFrec, datf, datg):
    ...
    # Primer ciclo para recores todas las fecuencias propuestas
    for j in range(len(datg[TipoFrec])):
        
        #Obtenemos los valores de frecuencia y potencia de la promera feciencia propuesta
        ...
        indis = set()
        #Recorremos todas las frecuencias conceciondas desde la mas cecana hacia abajo
        for i in range(PosNum, 0, -1):
            ...
            #Evaluamos si la propuesta esta por delate de la concecion
            elif valor - fr > 0:
        ...
        #Recorremos todas la frecuencias conceciondas desde la mas cecana hacia arriba
...
def RepoAnalisis( interferencia ):
    ...
    # ws['G3'].value = 'Concesiones / Autorizaciones'
    ...
    # ws['K3'].value = 'Interferencia lateral'
    # ws['K3'].alignment = Alignment(horizontal='center')

    # ws['K4'].value = 'Invación'
    # ws['L4'].value = "#'s de Id"
    ...
    for i in range(len(interferencia)):
        ...
        if(i%2 == 0 and i < len(interferencia)-1):
            ...
            # ws['K'+str(5+i-j)].value = str(interferencia[i+1])
            ws['L'+str(5+i-j)].value = str(interferencia[i+1][7])

            j=j+1

    #Guarda el libro en una secuencia (en lugar de guardar en un archivo en el disco) para enviarlo posteriormente al usuario
    # excel_stream = io.BytesIO()
    # wb.save(excel_stream)
    # excel_stream.seek(0)    # ir al principio de la corriente

    # return wb.save("Reporte.xlsx")
